...
def apply_isodata(images, params, limit=10):
    """
    Apply the Isodata algorithm and visualize the results for a given set of images.

    Parameters:
        images (ndarray): Array of gray-scale images to cluster.
        params (dict): Parameters for the Isodata algorithm.
        limit (int, optional): Maximum number of images to cluster and visualize. Defaults to 10.

    Returns:
        None

    """
    limit = limit if limit>0 else images.shape[0]
    for i in range(limit):
        ...
        plt.subplot(1, 3, 2)
        plt.imshow(class_image*img, cmap='gray')
        plt.xlabel("Clustered Image")
        
        plt.subplot(1, 3, 3)
        plt.imshow(class_image_eq, cmap='gray')
        plt.xlabel("Equilized Clustered Image")
...
def split_clusters(img_flat, img_class_flat, centers, clusters_list):
    ...
    avg_dists_to_clusters = compute_avg_distance(img_flat, img_class_flat, centers, clusters_list)
    d = compute_overall_distance(img_class_flat, avg_dists_to_clusters, clusters_list)
    ...
    if max_stddev > THETA_S:
        if avg_dists_to_clusters[cluster] >= d:
            if count_per_cluster[cluster] > (2.0 * THETA_M):
                ...
                clusters_list = np.append(clusters_list, [max_clusters_list, (max_clusters_list + 1)])

                centers, clusters_list = sort_arrays_by_first(centers, clusters_list)

                assert centers.size == clusters_list.size, "ERROR: split() centers and clusters_list size are different"

    return centers, clusters_list


def compute_overall_distance(img_class_flat, avg_dists_to_clusters, clusters_list):
    """
